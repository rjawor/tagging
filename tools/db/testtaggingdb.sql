...
CREATE TABLE IF NOT EXISTS `testtaggingdb`.`documents` (
  ...
  CONSTRAINT `fk_documents_folders1`
    ...
    REFERENCES `testtaggingdb`.`folders` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `testtaggingdb`.`help_sections`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `testtaggingdb`.`help_sections` ;

CREATE TABLE IF NOT EXISTS `testtaggingdb`.`help_sections` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `position` INT NULL,
  `text` TEXT NULL,
  PRIMARY KEY (`id`))
